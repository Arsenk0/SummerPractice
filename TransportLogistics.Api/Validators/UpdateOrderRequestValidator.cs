...
namespace TransportLogistics.Api.Validators
{
    public class UpdateOrderRequestValidator : AbstractValidator<UpdateOrderRequest>
    {
        public UpdateOrderRequestValidator()
        {
            ...
            RuleFor(x => x.ActualDeliveryDate)
                .GreaterThanOrEqualTo(x => x.ActualPickupDate).When(x => x.ActualDeliveryDate.HasValue && x.ActualPickupDate.HasValue)
    ...
    // ВИДАЛИТИ: Цей клас CargoRequestValidator був продубльований тут.
    // Він повинен бути визначений ТІЛЬКИ в CreateOrderRequestValidator.cs
    // Тому цей блок коду тут видаляється.
}
